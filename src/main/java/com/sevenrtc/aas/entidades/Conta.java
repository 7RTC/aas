...
public class Conta {
	...
	public static Conta load(ResultSet rs) {
		...
		try {
			c.codigo = rs.getString(1);
			c.contexto = rs.getLong(2);
			c.nome = rs.getString(3);
			c.saldo = rs.getDouble(4);
			c.tipo = rs.getString(5).charAt(0);
			c.funcao = rs.getString(6).charAt(0);
			c.pai = rs.getString(7);
			c.categoria_dre = rs.getLong(8);
	...
	public static void store(Conta conta) throws Exception {
		...
		strSQL = "INSERT INTO CON_Conta (CON_Codigo, CTX_ID, CON_Nome, CON_Saldo, CON_Tipo, CON_Funcao, CON_Pai, CAT_ID) "
				+ "VALUES ('"
				+ conta.codigo
				+ "', "
				+ conta.contexto
	 ...
	 * @param tipo
	 *            (A)tivo, (P)assivo, Patrimonio (L)iquido, (R)eceita ou
	 *            (D)espesa
	 * @param funcao
	 *            (S)intetica ou (A)nalitica
	 * @param pai
	 *            conta pai
	 * @param categoria_dre
	 *            Categoria na demonstração de resultados para contas do tipo
	 *            despesa ou receita
	 */
	public Conta(String codigo, long contexto, String nome, Number saldo,
			char tipo, char funcao, String pai, long categoria_dre) {
		super();
		this.codigo = codigo;
	...
	/**
	 * @param contexto
	 *            o numero do contexto ao qual ela pertence
	 */
	...
	/**
	 * @param funcao
	 *            a função da conta (analitica ou sintética)
	 */
	...
	/**
	 * @param pai
	 *            o pai da conta (null para nenhum)
	 */
