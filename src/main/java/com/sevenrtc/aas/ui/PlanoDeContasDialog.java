 ...
 * plano de contas de determina empresa. <p> O plano de contas é apresentado
 * como uma {@link javax.swing.JTree} construida a partir dos dados do banco na
 * classe {@link br.aas.comum.Contas}.
 *
...
public class PlanoDeContasDialog extends JDialog {
    ...
    private final Pattern ultimaConta = Pattern.compile("(\\d+)-");
     ...
     *
     * @param args argumentos passados como parametro
     */
     ...
     *
     * @param parent frame dona da atual
     */
    ...
    private void botaoAlterar_actionPerformed() {
        // Isola o nodo atual
        final DefaultMutableTreeNode nodoAtualizado = (DefaultMutableTreeNode) 
                arvoreContas.getLastSelectedPathComponent();

        String numeroConta = nodoAtualizado.toString();
    ...
    private void botaoIncluir_actionPerformed() {
        ...
        final int posicaoInsercao = encontraPosicaoNaArvore(nodoPai);
        // Invoca janela de manutencao
        ManutencaoContas m = new ManutencaoContas(this, numeroContaPai, 
                posicaoInsercao + 1, 0);
    ...
    /**
     * Inicializa a {@link javax.swing.JDialog} do plano de contas <p> A janela
     * é inicializada no centro da janela pai ocupando uma area de 60% do
     * tamanho da ultima
     *
     * @throws Exception
     */
    ...
    /**
     * Retorna o indice em que determinada {@link Conta} deve ser inserida
     * considerando seu no pai.
     *
     * @param pai conta analitica em que deve ser inserida a conta atual
     * @return um indice <tt>&gte; 0</tt>
     */
    private int encontraPosicaoNaArvore(DefaultMutableTreeNode pai) {
        int posicao = 0;
        final Enumeration irmaos = pai.children();
        int ultimaOrdem = 0;
        while (irmaos.hasMoreElements()) {
            final DefaultMutableTreeNode nodoFilho = (DefaultMutableTreeNode) irmaos.nextElement();
            final String contaIrma = (String) nodoFilho.getUserObject();
            final Matcher matcher = ultimaConta.matcher(contaIrma);
            if (matcher.find()) {
                final int ordemIrmao = Integer.parseInt(matcher.group(1));
                if (ordemIrmao - ultimaOrdem >= 2) {
                    break;
                }
                ultimaOrdem = ordemIrmao;
                posicao++;
            }
        }
        return posicao;
